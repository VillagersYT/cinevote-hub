...
import {
  addAdminMovie as addAdminMovieServer,
  deleteAdminOption,
  deleteAdminScreening,
  duplicateAdminScreening,
  getAdminDashboard,
  getAdminScreeningContent,
  resetAdminVotes,
  saveAdminScreening,
  saveAdminSettings,
  setAdminWinner,
...
function screeningToDraft(screening: Screening): ScreeningDraft {
  return {
    ...
    status:
      screening.status === "closed" || screening.status === "finished" ? screening.status : "open",
...
function AdminPage() {
  ...
  const contentQuery = useQuery({
    ...
    queryFn: () => {
      ...
      return getAdminScreeningContent({
        data: {
  ...
  const screenings = dashboardQuery.data?.screenings ?? [];
  ...
  const resetScreeningForm = () => {
    setEditingId(null);
    setScreeningDraft(createEmptyScreeningDraft(settings));
  };
  ...
  const saveScreening = useMutation({
    mutationFn: async () => {
      ...
      return saveAdminScreening({
        data: {
          screening: payload,
          screeningId: editingId,
        },
      });
    },
  ...
  const duplicateScreening = useMutation({
    mutationFn: async (screening: Screening) => {
      return duplicateAdminScreening({
        data: {
  ...
  const updateStatus = useMutation({
    mutationFn: ({ screeningId, status }: { screeningId: string; status: ScreeningStatus }) =>
      updateAdminScreeningStatus({
        data: {
          screeningId,
          status,
      ...
      return saveAdminSettings({
        data: {
          settings: {
            ...
            default_votes_per_voter: Math.max(
              1,
              Math.min(10, settingsDraft.default_votes_per_voter),
            ),
  ...
  const deleteOption = useMutation({
    mutationFn: ({ optionId }: { optionId: string; tmdbId: number }) => {
      if (!selectedScreeningId) {
        throw new Error("Aucune séance sélectionnée.");
      }

      return deleteAdminOption({
        data: {
          optionId,
          screeningId: selectedScreeningId,
        },
  ...
  const queryError = dashboardQuery.error ?? contentQuery.error;
      ...
      {activeTab === "screenings" && (
        <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)]">
          ...
          <section className="rounded-2xl border border-border bg-card p-6">
            ...
            {!dashboardQuery.isLoading && screenings.length === 0 && (
